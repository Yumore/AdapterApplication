...
public class AdapterLog extends CursorAdapter {
    ...
    public AdapterLog(Context context, Cursor cursor, boolean resolve, boolean organization) {
        ...
        iconSize = GuardUtils.dips2pixels(24, context);
    ...
    public void bindView(final View view, final Context context, final Cursor cursor) {
        ...
        // Show protocol name
        tvProtocol.setText(GuardUtils.getProtocolName(protocol, version, false));
        ...
        if (!we && organization) {
            if (!isKnownAddress(daddr))
                new AsyncTask<String, Object, String>() {
                    ...
                    protected String doInBackground(String... args) {
                        try {
                            return GuardUtils.getOrganization(args[0]);
                        } catch (Throwable ex) {
