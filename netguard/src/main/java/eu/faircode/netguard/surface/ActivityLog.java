...
import eu.faircode.netguard.adapter.AdapterLog;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lvLog.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                // Application name
                if (uid >= 0)
                    popup.getMenu().findItem(R.id.menu_application).setTitle(TextUtils.join(", ", GuardUtils.getApplicationNames(uid, ActivityLog.this)));
                else
                    popup.getMenu().removeItem(R.id.menu_application);

                // Destination IP
                popup.getMenu().findItem(R.id.menu_protocol).setTitle(GuardUtils.getProtocolName(protocol, version, false));
