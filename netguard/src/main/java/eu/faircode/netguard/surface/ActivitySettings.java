...
public class ActivitySettings extends AppCompatActivity implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        pref_reset_usage.setOnPreferenceClickListener(new Preference.OnPreferenceClickListener() {
            ...
            public boolean onPreferenceClick(Preference preference) {
                GuardUtils.areYouSure(ActivitySettings.this, R.string.setting_reset_usage, new GuardUtils.DoubtListener() {
                    @Override
                    public void onSure() {
        ...
        boolean can = GuardUtils.canFilter(this);
        ...
        if (GuardUtils.isPlayStoreInstall(this) || !GuardUtils.hasValidFingerprint(this))
        ...
        // Development
        if (!GuardUtils.isDebuggable(this))
    ...
    public void onSharedPreferenceChanged(SharedPreferences prefs, String name) {
        ...
        if ("screen_on".equals(name))
            ...
            try {
                if (!TextUtils.isEmpty(socks5_addr) && !GuardUtils.isNumericAddress(socks5_addr))
    ...
    private boolean checkPermissions(String name) {
        ...
        if ((name == null || "disable_on_call".equals(name)) && prefs.getBoolean("disable_on_call", false))
            if (!GuardUtils.hasPhoneStatePermission(this)) {
    ...
    private void checkAddress(String address, boolean allow_local) throws IllegalArgumentException, UnknownHostException {
        ...
        if (TextUtils.isEmpty(address))
            throw new IllegalArgumentException("Bad address");
        if (!GuardUtils.isNumericAddress(address))
            throw new IllegalArgumentException("Bad address");
        if (!allow_local) {
    ...
    private void updateTechnicalInfo() {
        ...
        pref_technical_info.setSummary(GuardUtils.getGeneralInfo(this));
        pref_technical_network.setSummary(GuardUtils.getNetworkInfo(this));
    }
    ...
    private void xmlImport(InputStream in) throws IOException, SAXException, ParserConfigurationException {
        ...
        ReceiverAutoRestart.upgrade(true, this);
