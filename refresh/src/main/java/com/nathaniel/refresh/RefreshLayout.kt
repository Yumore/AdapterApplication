 ...
 */
class RefreshLayout @JvmOverloads constructor(protected var mContext: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : ViewGroup(mContext, attrs, defStyleAttr) {
    //标记 无状态（既不是上拉 也 不是下拉）
    ...
    fun setHeaderView(headerView: View) {
        if (headerView is OnHeaderStateListener) {
            ...
            throw IllegalArgumentException("headerView must implement the OnHeaderStateListener")
        }
    }
    ...
    private fun initFooterLayout() {
        ...
        val lp = LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, LinearLayout.LayoutParams.WRAP_CONTENT)
    ...
    fun setFooterView(footerView: View) {
        if (footerView is OnFooterStateListener) {
            ...
            throw IllegalArgumentException("footerView must implement the OnFooterStateListener")
        }
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (mIsEmpty) {
            ...
            if (count > 3) {
                ...
                contentHeight = emptyView.measuredHeight + emptyLp.topMargin + emptyLp.bottomMargin
                contentWidth = emptyView.measuredWidth + emptyLp.leftMargin + emptyLp.rightMargin
            }
        } else {
            //内容布局容器
            if (count > 2) {
                ...
                contentHeight = content.measuredHeight + contentLp.topMargin + contentLp.bottomMargin
                contentWidth = content.measuredWidth + contentLp.leftMargin + contentLp.rightMargin
            }
        }
    ...
    override fun onInterceptTouchEvent(ev: MotionEvent): Boolean {
        ...
        when (ev.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                return if (pullRefresh() && y - mTouchY > mTouchSlop) {
                    true
                } else mHasMore && pullLoadMore() && mTouchY - y > mTouchSlop
            }
    ...
    private val isRefreshingOrLoading: Boolean
        private get() = mIsRefreshing || mIsLoadingMore
    ...
    private val isNestedScroll: Boolean
        private get() = isRefreshingOrLoading || !mHasMore
            ...
            return if (interval > 0) {
                ...
                if (scrollVelocity > 30) {
                    if (canPullDown() && mIsRefreshing && mFlingOrientation == ORIENTATION_FLING_UP) {
                        val height = headerTriggerHeight
                        smoothScroll(scrollY, -height, (1.0f * height * SCROLL_DELAY / scrollVelocity).toInt(),
                                false, null)
    ...
    fun autoRefresh() {
        ...
        post {
            mCurrentState = STATE_DOWN
            smoothScroll(scrollY, -headerTriggerHeight, 200, true, object : AnimatorListenerAdapter() {
    ...
    fun autoLoadMore() {
        ...
        post {
            mCurrentState = STATE_UP
            smoothScroll(scrollY, footerTriggerHeight, 200, true, object : AnimatorListenerAdapter() {
                override fun onAnimationEnd(animation: Animator) {
                    triggerLoadMore()
                }
            })
