...
public class MultiDexUtils {
    private static final String TAG = MultiDexUtils.class.getSimpleName();
    ...
    public static boolean isVMMultiDexCapable(String versionString) {
        ...
        if (versionString != null) {
            ...
            Matcher matcher = Pattern.compile(pattern).matcher(versionString);
            if (matcher.matches()) {
                try {
    ...
    public static void newTempFile(Context context) {
        try {
            ...
            if (!file.exists()) {
                boolean flags = file.createNewFile();
                Log.d(TAG, "newTempFile: " + flags);
            }
        } catch (Throwable th) {
            th.printStackTrace();
        }
    ...
    public static void preNewActivity() {
        long startTime = System.currentTimeMillis();
        Log.d(TAG, String.format("preNewActivity 耗时: %d", System.currentTimeMillis() - startTime));
    }
}
