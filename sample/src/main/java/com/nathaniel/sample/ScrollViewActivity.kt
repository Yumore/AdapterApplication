...
class ScrollViewActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val refreshTime = PreferencesUtils.getRefreshTime(S_REFRESH_TIME)
        if (refreshTime > 0) {
        ...
        mRefreshLayout!!.setOnRefreshListener(object : RefreshLayout.OnRefreshListener {
            override fun onRefresh() {
                ...
                mRefreshLayout!!.postDelayed({
                    PreferencesUtils.writeRefreshTime(S_REFRESH_TIME, Date().time)
                    //通知刷新完成
...
//        mRefreshLayout.hideEmpty();
    ...
    companion object {
        private const val S_REFRESH_TIME = "S_Refresh_Time"
    }
}